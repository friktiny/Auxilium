...
if (message.content.startsWith(`${BOT_PREFIX}getInfos`)) {
  if (mentionUser == undefined) {
    ...
    let infosUserEmbedMessage = new Discord.MessageEmbed().setColor(mentionUser.displayHexColor).setImage(mentionUser.user.displayAvatarURL({format : 'png', size : 256, dynamic : true})).setTitle(mentionUser.user.tag/*username + '#' + mentionUser.user.discriminator*/).setDescription(`Inscrit depuis le ${mentionUser.user.createdTimestamp}`).addField('Dernier message publié :', mentionUser.lastMessage + ` dans : ${mentionUser.lastMessage.channel.name}.`, true).addField('Activitée :', mentionUser.presence, true).addField('Est dans le serveur depuis :', mentionUser.joinedAt, false);

...
client.on('message', message => {
  ...
  if (message.content.startsWith(`${this.BOT_PREFIX}announcement`)) {
    if (!message.member.hasPermission('MENTION_EVERYONE')) {
      ...
      let annoucementSyntaxEmbedMessage = new Discord.MessageEmbed().setColor('BLACK').setTitle('Announcement Syntax').setDescription('`$announcement [channel where you want to send your message] [content of your message]`\n\n**Example**\n\n`$announcement #giveways @everyone Hi guys ! There a big giveways for the 6th anniversary of`' + message.guild.name + '` ! Get ready !`');
      message.channel.send(annoucementSyntaxEmbedMessage);
  ...
  if (message.content.startsWith(BOT_PREFIX + 'addnote')) {
    ...
    if(message.author.id !=='754229847206658160') {
      ...
      if (args[1] == null) {
        ...
        message.reply('The ' + args[1] + "th note does not exist!");
        } else {

...
if (message.content.startsWith(BOT_PREFIX + 'avatar')) {
  if (mentionUser == undefined) {
    ...
    var avatarYourSelfEmbedMessage = new Discord.MessageEmbed().setTitle(`L'avatar de ${mentionUser.displayName} :`).setImage(message.guild.members.cache.find(user => user.user.username === mentionUser.displayName).user.displayAvatarURL({format: 'png', size: '250px'})).setColor('#00ffff');

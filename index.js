...
if (message.content.startsWith(`${BOT_PREFIX}getInfos`)) {
  if (mentionUser == undefined) {
    ...
    let infosUserEmbedMessage = new Discord.MessageEmbed().setColor(mentionUser.displayHexColor).setImage(mentionUser.user.displayAvatarURL({format : 'png', size : 256, dynamic : true})).setTitle(mentionUser.user.tag).setDescription(`Inscrit depuis le ${mentionUser.user.createdAt}`).addField('Dernier message publié :', mentionUser.lastMessage.content, true).addField('Activitée :', mentionUser.user.presence, true).addField('Est dans le serveur depuis :', mentionUser.joinedAt, false);
